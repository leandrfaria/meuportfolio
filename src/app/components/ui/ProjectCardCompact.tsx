...
export default function ProjectCardCompact({ p, size = "md" }: Props) {
  ...
  return (
    <>
      ...
      >
        <div className={["relative w-full overflow-hidden rounded-lg bg-white/5", isSm ? "mb-2 aspect-[4/3] rounded-md" : "mb-3 aspect-[16/9]"].join(" ")}>
          {coverSrc ? (
            <Image
              src={coverSrc}
        ...
        <h3 className={isSm ? "text-[15px] font-semibold text-brand-white" : "text-base font-semibold text-brand-white"}>{p.title}</h3>

        <p className={["text-white/80", isSm ? "mt-1 line-clamp-2 text-[12px]" : "mt-1.5 line-clamp-3 text-[13px]"].join(" ")}>{p.summary}</p>

        <ul className={["flex flex-wrap", isSm ? "mt-2 gap-1" : "mt-3 gap-1.5"].join(" ")}>
          {p.tags.map((t) => (
            <li key={t} className={["rounded-md border border-white/10 bg-white/[0.04] text-white/85", isSm ? "px-1.5 py-[2px] text-[10px]" : "px-2 py-0.5 text-[11px]"].join(" ")}>
              {t}
            </li>
          ))}
        </ul>
        ...
        <div className={["flex items-center gap-2", isSm ? "mt-3" : "mt-4"].join(" ")}>
          <div className="flex items-center gap-2">
            {p.demoUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={["inline-flex items-center justify-center rounded-md bg-brand-green font-semibold text-brand-black transition hover:brightness-110", isSm ? "px-2.5 py-1 text-[11px]" : "px-3 py-1.5 text-[12px]"].join(" ")}
              >
                demo
              </a>
            )}
            {p.repoUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={["inline-flex items-center justify-center rounded-md border border-white/15 bg-white/[0.02] font-semibold text-brand-white transition hover:border-brand-green/70 hover:text-brand-green", isSm ? "px-2.5 py-1 text-[11px]" : "px-3 py-1.5 text-[12px]"].join(" ")}
              >
                repo
              </a>
            )}
          ...
          <div className="ml-auto">
            <button
              ...
              className={["inline-flex items-center justify-center rounded-md bg-white text-brand-black font-semibold transition hover:brightness-110", isSm ? "px-2.5 py-1 text-[11px]" : "px-3 py-1.5 text-[12px]"].join(" ")}
